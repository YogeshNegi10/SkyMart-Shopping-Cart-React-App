...
const Product = ({products}) => {
  ...
  const handleAdd = (item) =>{
   ...
   const {image,price,title,id} = products;

  return (
    <>
    <div className={`
     ${ show ? ' ' : '-z-10'}
    group relative border-gray-100/30 mb-4 flex w-full max-w-xs  flex-col self-center overflow-hidden rounded-lg border bg-gray-400 shadow-md`}>
      <Link
        className="relative mx-3 mt-3 flex h-60 overflow-hidden rounded-xl"
       to={`product/${id}`}
      >
        <img
          className="peer absolute top-0 right-0 h-[300px] w-[300px] object-contain"
          src={image}
          alt="product image"
        />
        {/* <img
